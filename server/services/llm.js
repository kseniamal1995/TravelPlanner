...
ФОТО (wiki): для каждого узнаваемого места (достопримечательность, музей, парк, храм, смотровая) укажи wiki — ТОЧНОЕ название статьи в русской Википедии (напр. «Колизей», «Лувр», «Сад Тюильри»). Для еды/логистики/безымянных перекусов можно опустить.
НАПОМИНАНИЯ: создавай reminders ТОЛЬКО для мест и событий, которые реально есть в этом маршруте (в days[].places) или среди забронированных мероприятий. Не добавляй напоминания про места, которых нет в плане. В тексте напоминания указывай день так, как он идёт в маршруте.`;
...
async function normalizePlace(p, bucket, order, city, cache = true) {
  ...
  const place = {
    ...
    pri: p.pri || undefined,
    rname,
    wiki: p.wiki || name, // фолбэк на имя — даёт шанс подтянуть фото из Википедии
